...
static rt_err_t nu_ui2c_wait_ready_with_timeout(nu_ui2c_t psNuUi2c)
{
    ...
    uint32_t u32ProtSts;
    while ((u32ProtSts = UI2C_GET_PROT_STATUS((UI2C_T *)psNuUi2c->m_module.base) &
...
{
    UI2C_SET_DATA((UI2C_T *)psNuUi2c->m_module.base, data);
    UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, UI2C_CTL_PTRG);
                                     ...
                                     struct rt_i2c_msg  *msg)
{
    ...
    {
        UI2C_ENABLE_10BIT_ADDR_MODE((UI2C_T *)psNuUi2c->m_module.base);
        ...
        if (flags & RT_I2C_RD)
        {
            ...
            UI2C_CLR_PROT_INT_FLAG((UI2C_T *)psNuUi2c->m_module.base, UI2C_PROTSTS_STARIF_Msk);
                                   ...
                                   rt_uint32_t num)
{
    ...
    /* Clear status */
    ((UI2C_T *)psNuUi2c->m_module.base)->PROTSTS = ((UI2C_T *)psNuUi2c->m_module.base)->PROTSTS;

    UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, UI2C_CTL_STA);
    ret = nu_ui2c_wait_ready_with_timeout(psNuUi2c);
    if (ret != RT_EOK) //for timeout condition
    {
    ...
    if (((UI2C_GET_PROT_STATUS((UI2C_T *)psNuUi2c->m_module.base) & UI2C_PROTSTS_STARIF_Msk) != UI2C_PROTSTS_STARIF_Msk))
    {
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (!(msg->flags & RT_I2C_NO_START))
        {
            if (i > 0)
            {
                UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, (UI2C_CTL_PTRG | UI2C_CTL_STA));/* Send repeat START */
                ret = nu_ui2c_wait_ready_with_timeout(psNuUi2c);
                if (ret != RT_EOK) //for timeout condition
                    break;

                if (((UI2C_GET_PROT_STATUS((UI2C_T *)psNuUi2c->m_module.base) & UI2C_PROTSTS_STARIF_Msk) != UI2C_PROTSTS_STARIF_Msk)) /* Check Send repeat START */
                {
                    i = 0;
        ...
        if (psNuUi2c->msg[i].flags & RT_I2C_RD) /* Receive Bytes */
        {
            ...
            for (cnt_data = 0 ; cnt_data < (psNuUi2c->msg[i].len) ; cnt_data++)
            {
                ...
                {
                    UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, (UI2C_CTL_PTRG | UI2C_CTL_AA));
                }
                ret = nu_ui2c_wait_ready_with_timeout(psNuUi2c);
                if (ret != RT_EOK)
                    break;

                if (((UI2C_T *)psNuUi2c->m_module.base)->PROTCTL & UI2C_CTL_AA)
                {
                ...
                else
                {
                    ...
                    if (((UI2C_GET_PROT_STATUS((UI2C_T *)psNuUi2c->m_module.base) & UI2C_PROTSTS_NACKIF_Msk) != UI2C_PROTSTS_NACKIF_Msk))
                    {
                        i = 0;
                        break;
                    }
                    UI2C_CLR_PROT_INT_FLAG((UI2C_T *)psNuUi2c->m_module.base, UI2C_PROTSTS_NACKIF_Msk);
                }

                psNuUi2c->msg[i].buf[cnt_data] = ((UI2C_T *)psNuUi2c->m_module.base)->RXDAT;
            }
        }
        else /* Send Bytes */
        {
    ...
    UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, (UI2C_CTL_PTRG | UI2C_CTL_STO));
    ret = nu_ui2c_wait_ready_with_timeout(psNuUi2c);
    if (ret != RT_EOK) //for timeout condition
    {
        rt_set_errno(-RT_ETIMEOUT);
    ...
    if (((UI2C_GET_PROT_STATUS((UI2C_T *)psNuUi2c->m_module.base) & UI2C_PROTSTS_STORIF_Msk) != UI2C_PROTSTS_STORIF_Msk))
    {
        i = 0;
        LOG_E("Send STOP Fail");
    }

    UI2C_CLR_PROT_INT_FLAG((UI2C_T *)psNuUi2c->m_module.base, UI2C_PROTSTS_STORIF_Msk);
    UI2C_SET_CONTROL_REG((UI2C_T *)psNuUi2c->m_module.base, UI2C_CTL_PTRG);
    UI2C_DISABLE_10BIT_ADDR_MODE((UI2C_T *)psNuUi2c->m_module.base); /*clear all sub modes like 10 bit mode*/
...
int rt_hw_ui2c_init(void)
{
    ...
    for (i = (UI2C_START + 1); i < UI2C_CNT; i++)
    {
        ...
        SYS_ResetModule(nu_ui2c_arr[i].m_module.RstId);
        UI2C_Close((UI2C_T *)nu_ui2c_arr[i].m_module.base);
        UI2C_Open((UI2C_T *)nu_ui2c_arr[i].m_module.base, 100000);
        nu_ui2c_arr[i].parent.ops = &nu_ui2c_ops;

        ret = rt_i2c_bus_device_register(&nu_ui2c_arr[i].parent, nu_ui2c_arr[i].m_module.name);
