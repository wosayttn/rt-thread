...
/**
 * @brief Erase a NAND block based on an absolute page address.
 * * @param qspi        Pointer to the QSPI device structure.
 * @param u32PageAddr Absolute page address (Full range across all Dies).
 * @return rt_err_t   RT_EOK on success, error code otherwise.
 */
static rt_err_t spinand_block_erase(struct rt_qspi_device *qspi, uint32_t u32PageAddr)
{
    rt_err_t result = RT_EOK;
    uint8_t u8WECmd = 0x06; // Write Enable Command
    uint8_t au8EraseCmd[4], u8SR;

    au8EraseCmd[0] = 0xD8;
    au8EraseCmd[1] = (uint8_t)((u32PageAddr >> 16) & 0xFF);
    au8EraseCmd[2] = (uint8_t)((u32PageAddr >> 8) & 0xFF);
    au8EraseCmd[3] = (uint8_t)(u32PageAddr & 0xFF);
...
static rt_err_t spinand_block_markbad(struct rt_qspi_device *qspi,  uint32_t u32PageAddr)
{
    ...
    result = spinand_block_erase(qspi, u32PageAddr);
    if (result != RT_EOK)
        return result;
