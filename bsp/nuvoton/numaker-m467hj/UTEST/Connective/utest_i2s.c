...
#if defined(NU_PKG_USING_NAU8822) && defined(PKG_USING_WAVPLAYER) && defined(BSP_USING_I2S0)
...
static void i2s_dump_test_setting(void)
{
    rt_kprintf("\n[I2S utest] description\n");
    rt_kprintf("  purpose               : Verify audio record/playback through I2S and NAU8822.\n");
...
{
    i2s_dump_test_setting();
    // Backup original multiple function pin setting.

    if (nu_hw_nau8822_init(&sCodecConfig_I2S) != RT_EOK)
        return -1;
...
{
    // Restore original multiple function pin setting.
...
UTEST_TC_EXPORT(testcase, UTEST_CMD_PREFIX"i2s", utest_tc_init, utest_tc_cleanup, 10);
